...
function calcIngredients() {
    ...
    if (foodItem && foodItem.ing) {
        ...
        document.getElementById('gold-cost').innerText = foodCost;
    }
}
...
function populateTabLeft() {
    ...
    if (chestSelectedTab === "Chests") {
        ...
        document.querySelector('.chests-right').style.display = 'flex';
        document.querySelector('.chest-list').style.display = 'flex';
        document.querySelector('.chests-left').style.background = "var(--backgroundTh)";
        ...
        document.querySelector('.chests-right').style.display = 'none';
        document.querySelector('.achiev-list').style.display = 'flex';

        document.querySelector('.chests-left').style.background = "repeating-linear-gradient(to bottom, #1b120e 0, #1b120e 60px, #1f1510 60px, #1f1510 120px)";
...
function populateChestInfo(island) {
    ...
    island.chests.forEach(chest => {
        ...
        const chestNumberDiv = document.createElement('div');
        chestNumberDiv.classList.add('chest-number-div');

        chestImg.appendChild(chestNumberDiv);
        const chestNumber = document.createElement('p');
        chestNumber.innerText = `${chest.id}`;
        chestNumber.classList.add('chest-number-info');
        chestDiv.appendChild(chestNumber);
        chestNumberDiv.appendChild(chestNumber);
        ...
        chest.items.forEach(e => {
            ...
            if (itemData) {
                ...
                if (e.quantity > 1) {
                    ...
                    itemQuantity.innerText = formatToK(e.quantity);
        ...
        console.log(chest.w)
        if (chest.w !== undefined) {
            const warningDiv = document.createElement('div');
            warningDiv.classList.add('chest-warning');
            warningDiv.innerHTML = "<p>!</p>";

            const warningTextDiv = document.createElement('div');
            const warningText = document.createElement('p');
            warningTextDiv.classList.add('warning-text-div');
            warningText.innerText = chest.w;

            warningTextDiv.appendChild(warningText);
            warningDiv.appendChild(warningTextDiv);

            warningTextDiv.style.display = "none";

            chestDiv.appendChild(warningDiv);
        }
...
chestMap.addEventListener('mousemove', (e) => {
    ...
    if (percentX >= 0 && percentX <= 100 && percentY >= 0 && percentY <= 100) {
        coordDisplay.innerText = `X: ${percentX.toFixed(2)}  Y: ${percentY.toFixed(2)} F: ${selectedFloor}`;
    } else {
        coordDisplay.innerText = '';
    }
    chestMap.addEventListener('click', () => {
        console.log(`X: ${percentX.toFixed(2)} Y: ${percentY.toFixed(2)} F: ${selectedFloor}`);
    });
});
...
document.querySelector('.update-text').innerText = updateText;
